...
- (void)start:(NSString *)url
{
    if (self.downLoadTask) {
        ...
        // 创建请求
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:self.mpSessionModel.urlString]];
        
        // 设置请求头
        NSString *range = [NSString stringWithFormat:@"bytes=%zd-", MPDownloadLength(self.mpSessionModel.urlString)];
        [request setValue:range forHTTPHeaderField:@"Range"];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveData:(NSData *)data
{
    ...
    self.mpSessionModel.process = progress;
        ...
        if (self.mpartnerDownloadProgressBlock) {
            self.mpartnerDownloadProgressBlock(progress,data.length,receivedSize,expectedSize);
