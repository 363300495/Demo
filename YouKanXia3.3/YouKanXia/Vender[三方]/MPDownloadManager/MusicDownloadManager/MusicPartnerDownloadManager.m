...
-(void)downLoad:(NSString *)url progressBlock:(MPartnerDownloadProgressBlock)progressBlock completeBlock:(MPartnerDownloadCompleteBlock)completeBlock{
    ...
    [downLoadTask downLoad:url progressBlock:^(CGFloat progress, int64_t bytes,CGFloat totalMBRead, CGFloat totalMBExpectedToRead) {
        
        
        dispatch_async(dispatch_get_main_queue(), ^{
            progressBlock(progress,bytes,totalMBRead,totalMBExpectedToRead);
        });
...
- (void)saveMPDownLoadTask{
    ...
    for (NSString *string in self.mpDownloadTasks.allKeys) {
        ...
        [mpSession setObject:dateString forKey:@"completeTime"];
