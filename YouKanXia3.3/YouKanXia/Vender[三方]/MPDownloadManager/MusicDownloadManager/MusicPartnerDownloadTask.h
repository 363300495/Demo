...
// 缓存主目录
#define MPCachesDirectory [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"MPCache"]

// 保存文件名
#define MPFileName(url) [url containsString:@".mp4"] ? [[url componentsSeparatedByString:@"/"] lastObject] : [NSString stringWithFormat:@"%@.mp4",[[url componentsSeparatedByString:@"/"] lastObject]]
...
typedef void(^MPartnerDownloadProgressBlock)(CGFloat progress,int64_t bytes ,CGFloat totalMBRead, CGFloat totalMBExpectedToRead);
