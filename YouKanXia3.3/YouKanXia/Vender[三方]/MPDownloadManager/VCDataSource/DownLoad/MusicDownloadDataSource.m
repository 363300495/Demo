...
-(void)startDownLoadUnFinishedTasks{
    ...
    for (TaskEntity *taskEntity in self.unFinishedTasks) {
        [[MusicPartnerDownloadManager sharedInstance] downLoad:taskEntity.downLoadUrl progressBlock:^(CGFloat progress, int64_t bytes,CGFloat totalMBRead, CGFloat totalMBExpectedToRead) {
            
            
            taskEntity.progress = progress;
            if (taskEntity.progressBlock) {
                 taskEntity.progressBlock(progress,bytes,totalMBRead,totalMBExpectedToRead);
            }
