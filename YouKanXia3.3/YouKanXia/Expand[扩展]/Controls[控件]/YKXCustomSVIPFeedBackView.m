...
@interface YKXCustomSVIPFeedBackView () <UITextViewDelegate>

@property (nonatomic,strong) UIButton *questionButotn1;

@property (nonatomic,strong) UIButton *questionButotn2;

@property (nonatomic,strong) UIButton *questionButotn3;
...
@implementation YKXCustomSVIPFeedBackView
...
- (instancetype)initWithPromptNote:(NSString *)promptNote loginContent:(NSString *)loginContent whiteContent:(NSString *)whiteContent sleepContent:(NSString *)sleepContent otherContent:(NSString *)otherContent{
    ...
    if(self = [super initWithFrame:SCREEN_BOUNDS]){
        ...
        UIView *alertView = [[UIView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-AlertViewWidth)/2.0, (SCREEN_HEIGHT-280)/2.0, AlertViewWidth, 320)];
        ...
        UIButton *questionButotn3 = [UIButton buttonWithType:UIButtonTypeCustom];
        questionButotn3.frame = CGRectMake((AlertViewWidth-200)/2, CGRectGetMaxY(questionButotn2.frame)+10, 200, 30);
        questionButotn3.layer.masksToBounds = YES;
        questionButotn3.layer.cornerRadius = 15;
        questionButotn3.layer.borderColor = [UIColor colorWithHexString:LIGHT_COLOR].CGColor;
        questionButotn3.layer.borderWidth = 0.6f;
        [questionButotn3 setTitle:sleepContent forState:UIControlStateNormal];
        [questionButotn3 setTitleColor:[UIColor colorWithHexString:DEEP_COLOR] forState:UIControlStateNormal];
        questionButotn3.titleLabel.font = [UIFont systemFontOfSize:14.0f];
        [questionButotn3 addTarget:self action:@selector(onClickButtonContent:) forControlEvents:UIControlEventTouchUpInside];
        [alertView addSubview:questionButotn3];
        self.questionButotn3 = questionButotn3;
        
        
        UILabel *otherLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, CGRectGetMaxY(questionButotn3.frame)+10, 200, 20)];
...
- (void)onClickSendMessage {
    ...
    if(!self.questionButotn1.selected && !self.questionButotn2.selected && !self.questionButotn3.selected){
        
        description = @"";
    }else if (self.questionButotn1.selected && !self.questionButotn2.selected && !self.questionButotn3.selected){
        
        description = @"3";
    }else if (self.questionButotn2.selected && !self.questionButotn1.selected && !self.questionButotn3.selected){
        
        description = @"4";
    }else if (self.questionButotn3.selected && !self.questionButotn1.selected && !self.questionButotn2.selected){
