...
- (void)createViewAtuoLayout{
    ...
    UIButton *cardUseButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [cardUseButton addTarget:self action:@selector(useSelf:) forControlEvents:UIControlEventTouchUpInside];
    [rightRootView addSubview:cardUseButton];
    [cardUseButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@(35));
        make.right.equalTo(rightRootView.mas_right).offset(-15);
        make.size.mas_equalTo(CGSizeMake(50,20));
    }];
    self.cardUseButton = cardUseButton;
    
    UIButton *donationButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [donationButton addTarget:self action:@selector(donationSelf:) forControlEvents:UIControlEventTouchUpInside];
    [rightRootView addSubview:donationButton];
    [donationButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(cardUseButton.mas_bottom).offset(15);
        make.right.equalTo(rightRootView.mas_right).offset(-15);
        make.size.mas_equalTo(CGSizeMake(50,20));
    }];
    self.donationButton = donationButton;
}


//使用卡券
- (void)useSelf:(UIButton *)btn{
    
    //标记使用过的卡券 滑动时复用问题 （使用时间大于0时标记为已经使用）
    _model.usetime = @"1";
    if (self.useBlock) {
        self.useBlock(btn);
    }
...
//赠送
- (void)donationSelf:(UIButton *)btn{
    if(self.donationBlock){
        self.donationBlock(btn);
    }
}
...
- (void)setModel:(CardListModel *)model{
    ...
    if([model.usetime integerValue] == 0){
        
        [self.cardUseButton setTitle:@"使用" forState:UIControlStateNormal];
        [self.cardUseButton setTitleColor:[UIColor colorWithHexString:@"#5484D7"] forState:UIControlStateNormal];
        self.cardUseButton.enabled = YES;
        self.cardUseButton.layer.borderColor = [UIColor colorWithHexString:@"#5484D7"].CGColor;
        self.cardUseButton.titleLabel.font = [UIFont systemFontOfSize:12.0f];
        self.cardUseButton.layer.masksToBounds = YES;
        self.cardUseButton.layer.cornerRadius = 10;
        self.cardUseButton.layer.borderWidth = 0.6;
        
    
        [self.donationButton setTitle:@"转赠" forState:UIControlStateNormal];
        [self.donationButton setTitleColor:[UIColor colorWithHexString:@"#5484D7"] forState:UIControlStateNormal];
        self.donationButton.enabled = YES;
        self.donationButton.layer.borderColor = [UIColor colorWithHexString:@"#5484D7"].CGColor;
        self.donationButton.titleLabel.font = [UIFont systemFontOfSize:12.0f];
        self.donationButton.layer.masksToBounds = YES;
        self.donationButton.layer.cornerRadius = 10;
        self.donationButton.layer.borderWidth = 0.6;
        self.donationButton.hidden = NO;
        
    }else{
        
        [self.cardUseButton setTitle:@"已使用" forState:UIControlStateNormal];
        [self.cardUseButton setTitleColor:[UIColor colorWithHexString:@"#A3A3A3"] forState:UIControlStateNormal];
        self.cardUseButton.enabled = NO;
        self.cardUseButton.layer.borderColor = [UIColor colorWithHexString:@"#A3A3A3"].CGColor;
        self.cardUseButton.titleLabel.font = [UIFont systemFontOfSize:12.0f];
        self.cardUseButton.layer.masksToBounds = YES;
        self.cardUseButton.layer.cornerRadius = 10;
        self.cardUseButton.layer.borderWidth = 0.6;
        self.donationButton.hidden = YES;
    }
}
