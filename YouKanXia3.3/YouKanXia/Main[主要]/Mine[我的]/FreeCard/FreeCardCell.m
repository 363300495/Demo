...
- (void)createViewAtuoLayout{
    ...
    UIButton *getCardButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [getCardButton addTarget:self action:@selector(onClickGetTicket:) forControlEvents:UIControlEventTouchUpInside];
    [rightRootView addSubview:getCardButton];
    [getCardButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(rightRootView);
        make.right.equalTo(rightRootView.mas_right).offset(-15);
        make.size.mas_equalTo(CGSizeMake(50,20));
    }];
    self.getCardButton = getCardButton;
}

- (void)onClickGetTicket:(UIButton *)btn{
    //通过receive来判断当前按钮是否被点击
    _model.receive = @"1";
    
    if(self.clickButtonAction){
        self.clickButtonAction(btn);
    }

}


- (void)setModel:(FreeCardModel *)model{
    
    _model = model;
    
    self.leftRootView.backgroundColor = [UIColor colorWithHexString:@"#5484D7"];
    ...
    self.lineImageView.image = [UIImage imageNamed:@"juan_line_get"];
    
    self.getCardImageView.image = [UIImage imageNamed:@"juan_tag_get"];
    ...
    self.getCardButton.titleLabel.font = [UIFont systemFontOfSize:12.0f];
    self.getCardButton.layer.masksToBounds = YES;
    self.getCardButton.layer.cornerRadius = 10;
    self.getCardButton.layer.borderWidth = 0.6;
    
    
    if([model.receive integerValue] == 0){
        [self.getCardButton setTitle:@"领取" forState:UIControlStateNormal];
        [self.getCardButton setTitleColor:[UIColor colorWithHexString:@"#5484D7"] forState:UIControlStateNormal];
        self.getCardButton.enabled = YES;
        self.getCardButton.layer.borderColor = [UIColor colorWithHexString:@"#5484D7"].CGColor;
    }else{
        [self.getCardButton setTitle:@"已领取" forState:UIControlStateNormal];
        [self.getCardButton setTitleColor:[UIColor colorWithHexString:@"#A3A3A3"] forState:UIControlStateNormal];
        self.getCardButton.enabled = NO;
        self.getCardButton.layer.borderColor = [UIColor colorWithHexString:@"#A3A3A3"].CGColor;
    }
}
